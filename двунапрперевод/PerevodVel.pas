...
begin
  ...
  if Sender = EVel1 then begin
    LError.Caption := '';
    if TryStrToFloat(EVel1.Text, f) then // Если удалось перевести текст в число, то
      EVel2.Text := FloatToStr(f*100000)
    else
      LError.Caption := 'Значение не обработано, пожалуйста, очистите поле и повторите попытку';
  end;
  // Берём данные EVel2, которого вызвал обработчик
  if Sender = EVel2 then begin
    LError.Caption := '';
    if TryStrToFloat(EVel2.Text, f) then // Если удалось перевести текст в число, то
